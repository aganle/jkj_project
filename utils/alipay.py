...
class AliPay(object):
    ...
    def direct_query(self,out_trade_no,trade_no):
        ...
        # {app_id:1231323,method:'saddsa',charset:'utf-8',biz_content；{subject:表一}}

        # import time
        # import requests
        # for i in range(10):
        #     response = requests.get('asd')
        #     if eval(response.text)['alipay_trade_query_response']['trade_staus'] == 'TRADE_SUCCESS':
        #         return True
        #     else:
        #         time.sleep(1)
    ...
    def sign_data(self, data):
        ...
        # 添加上‘’
        quoted_string = "&".join("{0}={1}".format(k, parse.quote_plus(v)) for k, v in unsigned_items)
